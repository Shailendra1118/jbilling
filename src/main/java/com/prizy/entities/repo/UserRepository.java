...
public class UserRepository implements IUserRepository {
	...
	public AppUser getUser(EntryPass creds) {
		...
		// TODO: this will be actually coming from database and not the actual
		// iteration of the user list

		for (AppUser user : appUserList) {
			if (user.getPass().getUname().equals(creds.getUname())
					&& user.getPass().getPwd().equals(creds.getPwd())) {
				return user;
			}
		}
